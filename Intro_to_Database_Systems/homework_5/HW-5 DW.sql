...
WHERE d.YEAR = '2015' AND d.MONTHNAME = 'September' AND f.salesprice > 20 AND f.quantity > 8
;
-- 2)
...
FROM dim_store s
	...
	JOIN dim_product p
		ON f.productid = p.productkey
	JOIN dim_date d
		ON d.datekey = f.salesdatekey 
WHERE d.YEAR = '2017' AND d.MONTHNAME = 'March' AND f.productcost < 50 AND s.city = 'Boulder'
;
-- 3)
SELECT s.salespersonname, SUM(f.SalesPrice * f.Quantity) AS revenue
...
FROM dim_store s 
	...
	JOIN dim_date d
		ON d.datekey = f.salesdatekey
WHERE d.YEAR BETWEEN 2010 AND 2017
GROUP BY s.storename
ORDER BY s.storename ASC
;
-- 6)
SELECT s.storename, p.productname, SUM((f.salesprice * f.quantity)-(f.productcost * f.quantity)) AS totalprofits
FROM dim_store s 
	JOIN fact_productsales f 
		ON s.storeid = f.storeid 
	JOIN dim_product p
	...
	JOIN dim_date d
		ON d.datekey = f.salesdatekey
WHERE d.YEAR = '2017' AND p.productname LIKE '%Jasmine Rice%'
GROUP BY s.storename
;
-- 7)
SELECT SUM(f.salesprice * f.quantity) AS totalrevenue, d.quarter
...
FROM dim_store s
	...
	JOIN dim_date d
		ON d.datekey = f.salesdatekey
WHERE d.DAYOFMONTH = '12' AND d.MONTHNAME = 'March' AND d.YEAR = '2017'
;
-- 10)
...
;
-- 11)
SELECT p.productname, SUM((f.salesprice * f.quantity)-(f.productcost * f.quantity)) AS totalprofits
FROM dim_product p
	JOIN fact_productsales f
...
FROM dim_date d
	JOIN fact_productsales f
		ON d.datekey = f.salesdatekey
WHERE d.YEAR = '2017' AND (d.MONTHNAME = 'January' OR d.MONTHNAME = 'February' OR d.MONTHNAME ='March')
GROUP BY d.monthname
;
-- 13. Display Product Name, average product cost and average sales price for the products sold in 
-- 2017. Show averages rounded to 2 decimal places.
-- 13)
SELECT p.productname, ROUND(AVG(f.productcost),2) AS avgcost, ROUND(AVG(f.salesprice),2) AS avgprice
FROM dim_product p 
	JOIN fact_productsales f
		ON p.productkey = f.productid
	JOIN dim_date d
...
;
-- 14)
SELECT c.customername, ROUND(AVG(f.salesprice),2) AS avgprice, ROUND(AVG(f.quantity),2) AS avgquantity
FROM dim_customer c 
	JOIN fact_productsales f
		ON c.customerid = f.customerid
...
;
-- 15)
SELECT s.storename, ROUND(MAX(f.salesprice),2) AS maxsalesprice, ROUND(MIN(salesprice),2) AS minsalesprice
FROM dim_store s 
	JOIN fact_productsales f
		ON s.storeid = f.storeid
