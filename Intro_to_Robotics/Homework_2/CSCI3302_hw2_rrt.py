...
def get_nearest_vertex(node_list, q_point):
    ...
    while i < len(node_list):
        c = np.linalg.norm(node_list[i].point - q_point)
        if c < q_near:
            near_index = i
        i += 1
    return node_list[near_index]
...
def steer(from_point, to_point, delta_q):
    ...
    a = np.linalg.norm(to_point - from_point)
    b = to_point - from_point
    c = (b/a) * delta_q
    new_point = c + from_point 
...
def check_path_valid(path, state_is_valid):
    ...
    i = 0
    while i < len(path):
        if state_is_valid(path[i]) == False:
            return False
        i += 1
    return True
...
def rrt(state_bounds, state_is_valid, starting_point, goal_point, k, delta_q):
    ...
    for i in range (k):
        q_point = get_random_valid_vertex(state_is_valid, state_bounds)
        if (goal_point is not None):
            if (random.random() < .05):
                q_point = goal_point
        q_near = get_nearest_vertex(node_list, q_point)
        q_path = steer(q_near.point, q_point, delta_q)
        if not check_path_valid(q_path, state_is_valid):
            continue
        q_new = Node(q_point, q_near)
        q_new.path_from_parent = q_path
