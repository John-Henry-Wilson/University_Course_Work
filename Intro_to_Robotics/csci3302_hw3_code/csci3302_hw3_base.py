...
def do_color_filtering(img):
  ...
  for index, pixel in np.ndenumerate(img):
    y, x, _ = index
    vals = img[y, x]
    if (check_if_color_in_range(vals) == True):
      mask[y,x] = 1
  return mask
...
def get_blobs(img_mask):
  ...
  #     TODO: Add blob_coords to blobs_list
        blobs_list.append(expand_nr(img_copy, (j,i), blob_coords))
        blob_coords.clear()
...
def main():
  ...
  # Examples:
  add_color_range_to_detect([0,8,137], [92,111,255]) # Detect red
  add_color_range_to_detect([58,180,19], [128,232,109]) # Detect green
  add_color_range_to_detect([212,89,15], [255,148,79]) # Detect blue
  add_color_range_to_detect([0,188,240], [53,245,255]) # Detect yellow
